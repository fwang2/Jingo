...
final class JingoMacUITests: XCTestCase {
  ...
  func testEmptyTranscriptCanvasAndNavigation() {
    ...
    app.buttons["Account"].click()
    XCTAssertTrue(element("account.iCloudBackup").waitForExistence(timeout: 2))
    XCTAssertTrue(app.staticTexts["Coming later"].exists)
    XCTAssertTrue(app.staticTexts["A future update will back up settings and recordings to your iCloud account."].exists)
