...
// MARK: - QwenModelMigration

struct QwenModelMigration: Migration {
  var version: Int { 3 }
  ...
  func migrate() throws {
    ...
    var settings = try Settings.fromFile(path: settingsURL.path)
    settings.selectedModelName = Model.defaultModelName
    settings.voiceLanguage = nil
    try settings.saveToFile(at: settingsURL)
  }
}
