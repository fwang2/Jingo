  ...
  var body: some ReducerOf<Self> {
    ...
    Reduce { state, action in
      ...
      case let .checkPurchaseStatus(isPurchased):
        state.$premiumFeatures.withLock { $0.liveTranscriptionIsPurchased = isPurchased }
        return .none

      case let .productFound(isFound):
        state.$premiumFeatures.withLock { $0.isProductFound = isFound }
        return .none
      }
    }
...
struct PremiumFeaturesSectionView: View {
  @Bindable var store: StoreOf<PremiumFeaturesSection>
