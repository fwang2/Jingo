  ...
  var body: some ReducerOf<Self> {
    Reduce<State, Action> { state, action in
      ...
      case let .purchaseResult(.success(isEnabled)):
        ...
        state.$premiumFeatures.withLock { $0.liveTranscriptionIsPurchased = isEnabled }
        return .send(.delegate(.didFinishTransaction))

      case let .purchaseResult(.failure(error)):
        state.isPurchasing = false
        state.errorMessage = error.localizedDescription
        state.$premiumFeatures.withLock { $0.liveTranscriptionIsPurchased = false }
        return .none
      ...
      case let .restorePurchaseCompleted(.success(isSubscribed)):
        ...
        if !isSubscribed {
          state.alert = AlertState {
            ...
            TextState("We couldn't find any purchases associated with your account.")
          }
        } else {
          state.$premiumFeatures.withLock { $0.liveTranscriptionIsPurchased = true }
          return .send(.delegate(.didFinishTransaction))
        }
        return .none
...
struct PurchaseLiveTranscriptionModalView: View {
  @Bindable var store: StoreOf<PurchaseLiveTranscriptionModal>
    ...
    Group {
      VStack(spacing: .grid(8)) {
        ...
        if store.isLoading {
          ...
          if store.restoreProgress.isIdle || store.restoreProgress.isError {
          ...
          HStack {
            Button(
              action: { store.send(.privacyPolicyTapped) },
              label: {
                Text("Privacy Policy")
            ...
            Button(
              action: { store.send(.termsOfUseTapped) },
              label: {
                Text("Terms of Use")
                  .foregroundColor(.DS.Text.subdued)
                  .textStyle(.captionBase)
