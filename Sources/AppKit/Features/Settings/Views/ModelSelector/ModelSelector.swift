...
struct ModelSelector {
  ...
  struct State: Equatable {
    ...
    var selectedModelID: Model.ID {
      ...
      set { $settings.withLock { $0.selectedModelName = newValue } }
    }
  ...
  var body: some ReducerOf<Self> {
    ...
    Reduce { state, action in
      ...
      case let .downloadTask(id: id, .success):
        state.$loadingProgress.withLock { $0 = nil }
      ...
      case let .modelsResponse(.success(newModels)):
        ...
        @Shared(.availableModels) var sharedModels
        ...
        if !models.contains(where: { $0.id == state.selectedModelID }),
           let defaultModel = models.first(where: { $0.model.isDefault }) {
  ...
  private func loadModel(state: inout State, id: Model.ID) -> Effect<Action> {
    state.loadQueue.remove(id)
    state.$loadingProgress.withLock { $0 = State.LoadingProgress(id: id, progress: 0) }
    return .run { [loadingProgress = state.$loadingProgress] send in
      await send(.downloadTask(id: id, TaskResult {
        try await transcriptionStream.loadModel(id) { progress in
          loadingProgress.withLock { $0?.progress = progress }
        }
...
struct ModelSelectorView: View {
  @Bindable var store: Store<ModelSelector.State, ModelSelector.Action>
...
private func modelAttributes(for model: Model) -> ModelSelector.State.ModelInfo {
  let isQwen = model.name == Model.qwen3ASR
  let isMultilingual = isQwen || !model.name.contains(".en")
  ...
  case Model.qwen3ASR:
    ...
    size = "710 MB"
    info = "Optimized for smooth Chinese and English transcription\nSupports 52 languages and dialects"
    position = 0
