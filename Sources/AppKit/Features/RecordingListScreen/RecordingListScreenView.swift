  ...
  var body: some Reducer<State, Action> {
    ...
    Reduce<State, Action> { state, action in
      ...
      case let .addFileRecordings(urls):
        return .run { send in
          await send(.binding(.set(\.isImportingFiles, true)), animation: .gentleBounce())
          ...
          await send(.binding(.set(\.isImportingFiles, false)), animation: .gentleBounce())
      ...
      case let .addRecordingInfo(recording):
        _ = state.$recordings.withLock { $0.insert(recording, at: 0) }
        return .none
      ...
      case let .alert(.presented(.deleteDialogConfirmed(id))),
        ...
        state.$recordings.withLock { $0.removeAll { $0.id == id } }
      ...
      case let .didSyncRecordings(.success(recordings)):
        state.$recordings.withLock { $0 = recordings.identifiedArray }
        return .none
  ...
  private func createCards(for state: State) -> IdentifiedArrayOf<RecordingCard.State> {
    state.recordings.elements
      .compactMap { recording in
        guard let sharedRecording = Shared(state.$recordings[id: recording.id]) else { return nil }
        return state.recordingCards[id: recording.id] ?? RecordingCard.State(
...
struct RecordingListScreenView: View {
  @Bindable var store: StoreOf<RecordingListScreen>
