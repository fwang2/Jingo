...
struct LiveTranscriptionModelSelectorView: View {
  @Bindable var store: StoreOf<LiveTranscriptionModelSelector>
        ...
        if isCheckingPurchase {
          ...
          VStack(spacing: .grid(1)) {
            ...
            VStack(alignment: .leading, spacing: .grid(2)) {
              Toggle(isOn: Binding(store.$settings).isLiveTranscriptionEnabled) {
                Label("Enable Live Transcription", systemImage: "text.viewfinder")
                  .textStyle(.body)
              }

              LabeledContent {
                Picker("", selection: Binding(store.$settings).selectedModelName) {
...
//            }
  ...
  private var isCheckingPurchase: Bool {
    #if APPSTORE
      store.state.premiumFeatures.liveTranscriptionIsPurchased == nil
    #else
      false
    #endif
  }
  ...
  private var isLocked: Bool {
    #if APPSTORE
      store.state.premiumFeatures.liveTranscriptionIsPurchased == false
    #else
      false
    #endif
  }
